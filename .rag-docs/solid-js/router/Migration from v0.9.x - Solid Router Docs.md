...
## [`<Outlet>`, `<Routes>`, `useRoutes`](https://docs.solidjs.com/solid-router/guides/migration#outlet-routes-useroutes)
...
With no `<Routes>` means the `<Router>` API has changed. The `<Router>` component acts as the `<Routes>` component now and its children must now be `<Route>` components. The top-level layout should go in the root prop of the router [as shown here](https://docs.solidjs.com/solid-router/concepts/layouts#root-level-layouts).

___

## [`element` prop removed from `Route`](https://docs.solidjs.com/solid-router/guides/migration#element-prop-removed-from-route)
...
## [`data` functions & `useRouteData`](https://docs.solidjs.com/solid-router/guides/migration#data-functions--useroutedata)
