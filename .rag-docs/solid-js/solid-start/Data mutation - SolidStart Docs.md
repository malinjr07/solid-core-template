Mutating data on a server is a common task in most applications. [Solid Router](https://docs.solidjs.com/solid-router) provides [actions](https://docs.solidjs.com/solid-router/concepts/actions) to manage data mutations effectively.
...
This page does not cover the foundational concepts from Solid Router. If you are a beginner, we highly recommend starting with the [actions documentation](https://docs.solidjs.com/solid-router/concepts/actions). You can also find many practical examples in the [data mutation how-to guide](https://docs.solidjs.com/solid-start/guides/data-mutation).

___

## [Server functions and actions](https://docs.solidjs.com/solid-start/building-your-application/data-mutation#server-functions-and-actions)
...
## [Single-flight mutations](https://docs.solidjs.com/solid-start/building-your-application/data-mutation#single-flight-mutations)
...
export default function ProductDetail(props: RouteSectionProps) {  const product = createAsync(() => getProductQuery(props.params.id as string));
  return (    <div>      <p>Current name: {props.data.product?.name}</p>      <form        action={updateProductAction.with(props.params.id as string)}        method="post"      >        <input name="name" placeholder="New name" />        <button>Save</button>      </form>    </div>  );}
```
